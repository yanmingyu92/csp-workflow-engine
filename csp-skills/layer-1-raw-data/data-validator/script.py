#!/usr/bin/env python3
"""DataValidator - Validate raw data quality (types, ranges, duplicates)."""
...
class DataValidator(BaseCSPSkill):
    """Validate raw data quality (types, ranges, duplicates)."""

    GRAPH_NODE_ID = "raw-data-validation"
    REQUIRED_INPUT_VARS = ["VARIABLE"]
    OUTPUT_VARS = ["VARIABLE", "CHECK_TYPE", "RESULT", "DETAILS"]
    SKILL_NAME = "data-validator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute data-validator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for data-validator.
...
def main():
    parser = create_argument_parser(
        "data-validator", "Validate raw data quality (types, ranges, duplicates)"
    )
