#!/usr/bin/env python3
"""CRFValidator - Validate CRF annotations against CDASH."""
...
class CRFValidator(BaseCSPSkill):
    """Validate CRF annotations against CDASH."""

    GRAPH_NODE_ID = "crf-validation"
    REQUIRED_INPUT_VARS = ["FIELD_NAME"]
    OUTPUT_VARS = ["FIELD_NAME", "MAPPING_STATUS", "VALIDATION_RESULT"]
    SKILL_NAME = "crf-validator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute crf-validator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for crf-validator.
...
def main():
    parser = create_argument_parser(
        "crf-validator", "Validate CRF annotations against CDASH"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = CRFValidator(config).run()
