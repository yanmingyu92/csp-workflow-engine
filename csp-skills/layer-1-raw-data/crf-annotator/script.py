#!/usr/bin/env python3
"""CRFAnnotator - Annotate blank CRF with SDTM variable targets."""
...
class CRFAnnotator(BaseCSPSkill):
    """Annotate blank CRF with SDTM variable targets."""

    GRAPH_NODE_ID = "crf-validation"
    REQUIRED_INPUT_VARS = ["CRF_FIELD"]
    OUTPUT_VARS = ["CRF_FIELD", "SDTM_DOMAIN", "SDTM_VARIABLE"]
    SKILL_NAME = "crf-annotator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute crf-annotator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for crf-annotator.
...
def main():
    parser = create_argument_parser(
        "crf-annotator", "Annotate blank CRF with SDTM variable targets"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = CRFAnnotator(config).run()
