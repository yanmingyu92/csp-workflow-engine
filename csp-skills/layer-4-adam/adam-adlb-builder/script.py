#!/usr/bin/env python3
"""ADLBBuilder - Build ADLB laboratory analysis dataset (BDS)."""
...
class ADLBBuilder(BaseCSPSkill):
    """Build ADLB laboratory analysis dataset (BDS)."""

    GRAPH_NODE_ID = "adam-adlb"
    REQUIRED_INPUT_VARS = ["USUBJID", "PARAMCD", "AVISIT"]
    OUTPUT_VARS = ["STUDYID", "USUBJID", "PARAMCD", "AVAL", "BASE", "CHG", "ABLFL"]
    SKILL_NAME = "adam-adlb-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute adam-adlb-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for adam-adlb-builder.
...
def main():
    parser = create_argument_parser(
        "adam-adlb-builder", "Build ADLB laboratory analysis dataset (BDS)"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = ADLBBuilder(config).run()
