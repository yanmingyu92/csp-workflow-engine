#!/usr/bin/env python3
"""ADAMCustomBuilder - Build custom ADaM datasets (PK, QoL, PRO)."""
...
class ADAMCustomBuilder(BaseCSPSkill):
    """Build custom ADaM datasets (PK, QoL, PRO)."""

    GRAPH_NODE_ID = "adam-custom-analysis"
    REQUIRED_INPUT_VARS = ["USUBJID"]
    OUTPUT_VARS = ["STUDYID", "USUBJID", "PARAMCD", "AVAL"]
    SKILL_NAME = "adam-custom-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute adam-custom-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for adam-custom-builder.
...
def main():
    parser = create_argument_parser(
        "adam-custom-builder", "Build custom ADaM datasets (PK, QoL, PRO)"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = ADAMCustomBuilder(config).run()
