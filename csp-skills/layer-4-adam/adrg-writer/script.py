#!/usr/bin/env python3
"""ADRGWriter - Write ADaM Reviewers Guide (ADRG)."""
...
class ADRGWriter(BaseCSPSkill):
    """Write ADaM Reviewers Guide (ADRG)."""

    GRAPH_NODE_ID = "adam-reviewer-guide"
    REQUIRED_INPUT_VARS = ["ADAM_PATH", "TRACEABILITY_REPORT"]
    OUTPUT_VARS = ["SECTION", "CONTENT"]
    SKILL_NAME = "adrg-writer"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute adrg-writer skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for adrg-writer.
...
def main():
    parser = create_argument_parser("adrg-writer", "Write ADaM Reviewers Guide (ADRG)")
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = ADRGWriter(config).run()
