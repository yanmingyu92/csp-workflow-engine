#!/usr/bin/env python3
"""ADEFFBuilder - Build ADEFF efficacy analysis dataset."""
...
class ADEFFBuilder(BaseCSPSkill):
    """Build ADEFF efficacy analysis dataset."""

    GRAPH_NODE_ID = "adam-adeff"
    REQUIRED_INPUT_VARS = ["USUBJID", "PARAMCD"]
    OUTPUT_VARS = ["STUDYID", "USUBJID", "PARAMCD", "PARAM", "AVAL", "AVALC"]
    SKILL_NAME = "adam-adeff-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute adam-adeff-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for adam-adeff-builder.
...
def main():
    parser = create_argument_parser(
        "adam-adeff-builder", "Build ADEFF efficacy analysis dataset"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = ADEFFBuilder(config).run()
