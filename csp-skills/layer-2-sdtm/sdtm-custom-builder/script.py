#!/usr/bin/env python3
"""SDTMCustomBuilder - Build custom SDTM domains (FA, QS, biomarkers)."""
...
class SDTMCustomBuilder(BaseCSPSkill):
    """Build custom SDTM domains (FA, QS, biomarkers)."""

    GRAPH_NODE_ID = "sdtm-custom-domain"
    REQUIRED_INPUT_VARS = ["STUDYID", "SUBJID", "DOMAIN"]
    OUTPUT_VARS = ["STUDYID", "DOMAIN", "USUBJID", "SEQ", "TESTCD", "ORRES"]
    SKILL_NAME = "sdtm-custom-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdtm-custom-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdtm-custom-builder.
...
def main():
    parser = create_argument_parser(
        "sdtm-custom-builder", "Build custom SDTM domains (FA, QS, biomarkers)"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDTMCustomBuilder(config).run()
