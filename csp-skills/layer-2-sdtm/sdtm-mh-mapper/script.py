#!/usr/bin/env python3
"""SDTMMHMapper - Map medical history to SDTM MH domain."""
...
class SDTMMHMapper(BaseCSPSkill):
    """Map medical history to SDTM MH domain."""

    GRAPH_NODE_ID = "sdtm-mh-mapping"
    REQUIRED_INPUT_VARS = ["STUDYID", "SUBJID", "MHTERM"]
    OUTPUT_VARS = ["STUDYID", "DOMAIN", "USUBJID", "MHSEQ", "MHTERM", "MHDECOD"]
    SKILL_NAME = "sdtm-mh-mapper"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdtm-mh-mapper skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdtm-mh-mapper.
...
def main():
    parser = create_argument_parser(
        "sdtm-mh-mapper", "Map medical history to SDTM MH domain"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDTMMHMapper(config).run()
