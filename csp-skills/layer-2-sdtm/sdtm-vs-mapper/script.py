#!/usr/bin/env python3
"""SDTMVSMapper - Map vital signs to SDTM VS domain."""
...
class SDTMVSMapper(BaseCSPSkill):
    """Map vital signs to SDTM VS domain."""

    GRAPH_NODE_ID = "sdtm-vs-mapping"
    REQUIRED_INPUT_VARS = ["STUDYID", "SUBJID", "VSTEST", "VSORRES"]
    OUTPUT_VARS = ["STUDYID", "DOMAIN", "USUBJID", "VSSEQ", "VSTEST", "VSSTRESN"]
    SKILL_NAME = "sdtm-vs-mapper"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdtm-vs-mapper skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdtm-vs-mapper.
...
def main():
    parser = create_argument_parser(
        "sdtm-vs-mapper", "Map vital signs to SDTM VS domain"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDTMVSMapper(config).run()
