#!/usr/bin/env python3
"""SDTMSVMapper - Map subject visits to SDTM SV domain."""
...
class SDTMSVMapper(BaseCSPSkill):
    """Map subject visits to SDTM SV domain."""

    GRAPH_NODE_ID = "sdtm-sv-mapping"
    REQUIRED_INPUT_VARS = ["STUDYID", "SUBJID", "VISIT", "SVSTDTC"]
    OUTPUT_VARS = ["STUDYID", "DOMAIN", "USUBJID", "VISIT", "VISITNUM", "SVSTDTC"]
    SKILL_NAME = "sdtm-sv-mapper"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdtm-sv-mapper skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdtm-sv-mapper.
...
def main():
    parser = create_argument_parser(
        "sdtm-sv-mapper", "Map subject visits to SDTM SV domain"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDTMSVMapper(config).run()
