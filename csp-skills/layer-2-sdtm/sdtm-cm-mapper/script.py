#!/usr/bin/env python3
"""SDTMCMMapper - Map concomitant medication data to SDTM CM domain."""
...
class SDTMCMMapper(BaseCSPSkill):
    """Map concomitant medication data to SDTM CM domain."""

    GRAPH_NODE_ID = "sdtm-cm-mapping"
    REQUIRED_INPUT_VARS = ["STUDYID", "SUBJID", "CMTRT"]
    OUTPUT_VARS = ["STUDYID", "DOMAIN", "USUBJID", "CMSEQ", "CMTRT", "CMDECOD"]
    SKILL_NAME = "sdtm-cm-mapper"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdtm-cm-mapper skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdtm-cm-mapper.
...
def main():
    parser = create_argument_parser(
        "sdtm-cm-mapper", "Map concomitant medication data to SDTM CM domain"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDTMCMMapper(config).run()
