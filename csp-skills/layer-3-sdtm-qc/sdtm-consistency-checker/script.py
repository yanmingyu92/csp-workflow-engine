#!/usr/bin/env python3
"""SDTMConsistencyChecker - Check cross-domain consistency for SDTM."""
...
class SDTMConsistencyChecker(BaseCSPSkill):
    """Check cross-domain consistency for SDTM."""

    GRAPH_NODE_ID = "sdtm-crossdomain-check"
    REQUIRED_INPUT_VARS = ["SDTM_PATH"]
    OUTPUT_VARS = ["CHECK_ID", "DOMAINS", "RESULT", "DETAILS"]
    SKILL_NAME = "sdtm-consistency-checker"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdtm-consistency-checker skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdtm-consistency-checker.
...
def main():
    parser = create_argument_parser(
        "sdtm-consistency-checker", "Check cross-domain consistency for SDTM"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDTMConsistencyChecker(config).run()
