#!/usr/bin/env python3
"""P21Validator - Run Pinnacle 21 validation on datasets."""
...
class P21Validator(BaseCSPSkill):
    """Run Pinnacle 21 validation on datasets."""

    GRAPH_NODE_ID = "p21-sdtm-validation"
    REQUIRED_INPUT_VARS = ["DATASETS_PATH"]
    OUTPUT_VARS = ["RULE_ID", "SEVERITY", "DOMAIN", "VARIABLE", "MESSAGE"]
    SKILL_NAME = "p21-validator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute p21-validator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for p21-validator.
...
def main():
    parser = create_argument_parser(
        "p21-validator", "Run Pinnacle 21 validation on datasets"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = P21Validator(config).run()
