#!/usr/bin/env python3
"""SDRGWriter - Write SDTM Reviewers Guide (SDRG)."""
...
class SDRGWriter(BaseCSPSkill):
    """Write SDTM Reviewers Guide (SDRG)."""

    GRAPH_NODE_ID = "sdtm-reviewer-guide"
    REQUIRED_INPUT_VARS = ["SDTM_PATH", "P21_REPORT"]
    OUTPUT_VARS = ["SECTION", "CONTENT"]
    SKILL_NAME = "sdrg-writer"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute sdrg-writer skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for sdrg-writer.
...
def main():
    parser = create_argument_parser("sdrg-writer", "Write SDTM Reviewers Guide (SDRG)")
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SDRGWriter(config).run()
