#!/usr/bin/env python3
"""DefineDraftBuilder - Generate draft Define.xml metadata for SDTM."""
...
class DefineDraftBuilder(BaseCSPSkill):
    """Generate draft Define.xml metadata for SDTM."""

    GRAPH_NODE_ID = "sdtm-define-draft"
    REQUIRED_INPUT_VARS = ["SDTM_PATH"]
    OUTPUT_VARS = ["DATASET", "VARIABLE", "LABEL", "TYPE", "LENGTH"]
    SKILL_NAME = "define-draft-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute define-draft-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for define-draft-builder.
...
def main():
    parser = create_argument_parser(
        "define-draft-builder", "Generate draft Define.xml metadata for SDTM"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = DefineDraftBuilder(config).run()
