#!/usr/bin/env python3
"""P21ReportReviewer - Review and categorize P21 validation issues."""
...
class P21ReportReviewer(BaseCSPSkill):
    """Review and categorize P21 validation issues."""

    GRAPH_NODE_ID = "p21-sdtm-validation"
    REQUIRED_INPUT_VARS = ["REPORT_PATH"]
    OUTPUT_VARS = ["ISSUE_ID", "RESOLUTION", "STATUS", "NOTES"]
    SKILL_NAME = "p21-report-reviewer"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute p21-report-reviewer skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for p21-report-reviewer.
...
def main():
    parser = create_argument_parser(
        "p21-report-reviewer", "Review and categorize P21 validation issues"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = P21ReportReviewer(config).run()
