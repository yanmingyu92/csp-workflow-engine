#!/usr/bin/env python3
"""TFLShellReviewer - Review TFL shells from SAP and set up templates."""
...
class TFLShellReviewer(BaseCSPSkill):
    """Review TFL shells from SAP and set up templates."""

    GRAPH_NODE_ID = "tfl-shell-review"
    REQUIRED_INPUT_VARS = ["SHELL_PATH"]
    OUTPUT_VARS = ["TFL_ID", "TITLE", "STATUS", "TEMPLATE_PATH"]
    SKILL_NAME = "tfl-shell-reviewer"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-shell-reviewer skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-shell-reviewer.
...
def main():
    parser = create_argument_parser(
        "tfl-shell-reviewer", "Review TFL shells from SAP and set up templates"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLShellReviewer(config).run()
