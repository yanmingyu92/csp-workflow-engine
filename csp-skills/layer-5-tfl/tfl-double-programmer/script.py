#!/usr/bin/env python3
"""TFLDoubleProgrammer - Independent double programming for key outputs."""
...
class TFLDoubleProgrammer(BaseCSPSkill):
    """Independent double programming for key outputs."""

    GRAPH_NODE_ID = "tfl-double-programming"
    REQUIRED_INPUT_VARS = ["ADAM_PATH", "SPEC_PATH"]
    OUTPUT_VARS = ["TFL_ID", "MATCH", "ROOT_CAUSE"]
    SKILL_NAME = "tfl-double-programmer"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-double-programmer skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-double-programmer.
...
def main():
    parser = create_argument_parser(
        "tfl-double-programmer", "Independent double programming for key outputs"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLDoubleProgrammer(config).run()
