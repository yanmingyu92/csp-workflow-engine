#!/usr/bin/env python3
"""TFLFigureGenerator - Generate analysis figures (KM curves, forest plots)."""
...
class TFLFigureGenerator(BaseCSPSkill):
    """Generate analysis figures (KM curves, forest plots)."""

    GRAPH_NODE_ID = "tfl-figure-generation"
    REQUIRED_INPUT_VARS = ["ADAM_PATH", "TEMPLATE_PATH"]
    OUTPUT_VARS = ["FIGURE_ID", "OUTPUT_PATH", "STATUS"]
    SKILL_NAME = "tfl-figure-generator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-figure-generator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-figure-generator.
...
def main():
    parser = create_argument_parser(
        "tfl-figure-generator", "Generate analysis figures (KM curves, forest plots)"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLFigureGenerator(config).run()
