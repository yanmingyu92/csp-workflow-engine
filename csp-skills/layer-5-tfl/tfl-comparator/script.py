#!/usr/bin/env python3
"""TFLComparator - Compare TFL production vs QC outputs."""
...
class TFLComparator(BaseCSPSkill):
    """Compare TFL production vs QC outputs."""

    GRAPH_NODE_ID = "tfl-qc-validation"
    REQUIRED_INPUT_VARS = ["PRODUCTION_PATH", "QC_PATH"]
    OUTPUT_VARS = ["TFL_ID", "MATCH_STATUS", "DIFF_COUNT", "DIFF_DETAILS"]
    SKILL_NAME = "tfl-comparator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-comparator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-comparator.
...
def main():
    parser = create_argument_parser(
        "tfl-comparator", "Compare TFL production vs QC outputs"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLComparator(config).run()
