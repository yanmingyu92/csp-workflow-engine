#!/usr/bin/env python3
"""TFLTemplateBuilder - Build reusable TFL programming templates."""
...
class TFLTemplateBuilder(BaseCSPSkill):
    """Build reusable TFL programming templates."""

    GRAPH_NODE_ID = "tfl-shell-review"
    REQUIRED_INPUT_VARS = ["SHELL_PATH"]
    OUTPUT_VARS = ["TEMPLATE_ID", "TEMPLATE_PATH", "TYPE"]
    SKILL_NAME = "tfl-template-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-template-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-template-builder.
...
def main():
    parser = create_argument_parser(
        "tfl-template-builder", "Build reusable TFL programming templates"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLTemplateBuilder(config).run()
