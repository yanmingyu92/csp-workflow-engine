#!/usr/bin/env python3
"""TFLListingGenerator - Generate patient data listings."""
...
class TFLListingGenerator(BaseCSPSkill):
    """Generate patient data listings."""

    GRAPH_NODE_ID = "tfl-listing-generation"
    REQUIRED_INPUT_VARS = ["ADAM_PATH", "TEMPLATE_PATH"]
    OUTPUT_VARS = ["LISTING_ID", "OUTPUT_PATH", "STATUS"]
    SKILL_NAME = "tfl-listing-generator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-listing-generator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-listing-generator.
...
def main():
    parser = create_argument_parser(
        "tfl-listing-generator", "Generate patient data listings"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLListingGenerator(config).run()
