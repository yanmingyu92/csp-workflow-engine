#!/usr/bin/env python3
"""TFLTableGenerator - Generate analysis tables from ADaM datasets."""
...
class TFLTableGenerator(BaseCSPSkill):
    """Generate analysis tables from ADaM datasets."""

    GRAPH_NODE_ID = "tfl-table-generation"
    REQUIRED_INPUT_VARS = ["ADAM_PATH", "TEMPLATE_PATH"]
    OUTPUT_VARS = ["TABLE_ID", "OUTPUT_PATH", "STATUS"]
    SKILL_NAME = "tfl-table-generator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute tfl-table-generator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for tfl-table-generator.
...
def main():
    parser = create_argument_parser(
        "tfl-table-generator", "Generate analysis tables from ADaM datasets"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = TFLTableGenerator(config).run()
