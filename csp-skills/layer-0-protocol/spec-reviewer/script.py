#!/usr/bin/env python3
"""SpecReviewer - Review and validate mapping specifications."""
...
class SpecReviewer(BaseCSPSkill):
    """Review and validate mapping specifications."""

    GRAPH_NODE_ID = "spec-creation"
    REQUIRED_INPUT_VARS = ["SPEC_PATH"]
    OUTPUT_VARS = ["SPEC_SECTION", "STATUS", "COMMENTS"]
    SKILL_NAME = "spec-reviewer"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute spec-reviewer skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for spec-reviewer.
...
def main():
    parser = create_argument_parser(
        "spec-reviewer", "Review and validate mapping specifications"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = SpecReviewer(config).run()
