#!/usr/bin/env python3
"""DefineXMLValidator - Validate Define.xml against CDISC schema."""
...
class DefineXMLValidator(BaseCSPSkill):
    """Validate Define.xml against CDISC schema."""

    GRAPH_NODE_ID = "define-xml-sdtm"
    REQUIRED_INPUT_VARS = ["DEFINE_PATH"]
    OUTPUT_VARS = ["CHECK_ID", "RESULT", "DETAILS"]
    SKILL_NAME = "define-xml-validator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute define-xml-validator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for define-xml-validator.
...
def main():
    parser = create_argument_parser(
        "define-xml-validator", "Validate Define.xml against CDISC schema"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = DefineXMLValidator(config).run()
