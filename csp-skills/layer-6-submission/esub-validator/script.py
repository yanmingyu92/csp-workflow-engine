#!/usr/bin/env python3
"""ESubValidator - Validate eCTD submission package."""
...
class ESubValidator(BaseCSPSkill):
    """Validate eCTD submission package."""

    GRAPH_NODE_ID = "esub-package-assembly"
    REQUIRED_INPUT_VARS = ["ESUB_PATH"]
    OUTPUT_VARS = ["CHECK_ID", "RESULT", "SEVERITY", "DETAILS"]
    SKILL_NAME = "esub-validator"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute esub-validator skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for esub-validator.
...
def main():
    parser = create_argument_parser(
        "esub-validator", "Validate eCTD submission package"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = ESubValidator(config).run()
