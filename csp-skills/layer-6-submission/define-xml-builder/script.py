#!/usr/bin/env python3
"""DefineXMLBuilder - Generate final Define.xml for datasets."""
...
class DefineXMLBuilder(BaseCSPSkill):
    """Generate final Define.xml for datasets."""

    GRAPH_NODE_ID = "define-xml-sdtm"
    REQUIRED_INPUT_VARS = ["DATASETS_PATH", "SPEC_PATH"]
    OUTPUT_VARS = ["DEFINE_XML_PATH", "SCHEMA_VALID", "ISSUES"]
    SKILL_NAME = "define-xml-builder"
    SKILL_VERSION = "1.0.0"

    def run(self) -> SkillResult:
        """Execute define-xml-builder skill."""
    ...
    def _execute(self, input_path: Path, output_path: Path) -> Dict[str, Any]:
        """
        Core execution logic for define-xml-builder.
...
def main():
    parser = create_argument_parser(
        "define-xml-builder", "Generate final Define.xml for datasets"
    )
    args = parser.parse_args()
    config = SkillConfig.from_args(args)
    result = DefineXMLBuilder(config).run()
