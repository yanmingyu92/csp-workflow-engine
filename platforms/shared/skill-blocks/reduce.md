...
# {categories_table} is populated from the preset's categories.yaml.
source_skill: reduce
min_processing_depth: light
platform: shared
---
...
- notes folder: `{config.notes_dir}/`
- inbox folder: `{config.inbox_dir}/`
...
|---------|---------------|---------|
| **Having knowledge** | The vault contains information | "We store {vocabulary.note_plural} in folders" |
...
- If target is empty: scan {config.inbox_dir}/ for unprocessed items, pick one
...
| Observation | Action |
|-------------|--------|
| Any observation | Create atomic note in `{config.ops_dir}/observations/` with prose-sentence title |
| Tension: content contradicts existing {vocabulary.note} | Create atomic note in `{config.ops_dir}/tensions/` with prose-sentence title |

The handoff Learnings section summarizes what you ALREADY logged during processing.

---

# {vocabulary.reduce_title}

Extract composable {vocabulary.note_plural} from source material into {config.notes_dir}/.
...
|---------|---------|-----------------|
| **We DO this** | "We tag {vocabulary.note_plural} with topics" | — (not sufficient) |
...
- BASIC thinking: "Is this a standalone composable {vocabulary.note}?"
...
```bash
# Get descriptions from existing {vocabulary.note_plural}
for f in {config.notes_dir}/*.md; do
...
grep -ril "[key terms from claim]" {config.notes_dir}/ --include="*.md" | head -5
```

Keyword search checks for exact term matches. It misses same-concept-different-words duplicates but is fast and requires no external tools.
{endif}
...
Write to: `{config.notes_dir}/[title].md`
...
- The chain: research query -> inbox file -> /{vocabulary.reduce} -> {config.notes_dir}
...
After extraction, for EACH claim, create a task file in `{config.ops_dir}/queue/`:
...
For each ENRICHMENT detected, create a task file in `{config.ops_dir}/queue/`:
...
After creating task files, update `{config.ops_dir}/queue/queue.json`:
...
```
=== RALPH HANDOFF: {vocabulary.reduce} ===
Target: [source file]
...
Files Modified:
- {config.ops_dir}/queue/{source}-NNN.md (claim files)
- {config.ops_dir}/queue/{source}-EEE.md (enrichment files, if any)
- {config.ops_dir}/queue/queue.json (N claim tasks + M enrichment tasks, 1 entry each)
...
- **suggested:** Output next step AND add each created {vocabulary.note} to `{config.ops_dir}/queue/queue.json` with `current_phase: "create"` and `completed_phases: []`
